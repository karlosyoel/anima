...
<html lang="en">
    ...
    <body>
		<div class="el">
            <button id="export">Save</button> | <button id="load">Load map</button> | <button id="reload">Reload</button>
        </div>
		<script src="./vendor/mrdoob/three.js/examples/js/libs/ammo.wasm.js"></script>
		<script src="otros/jquery-git.min.js"></script>
        ...
        <script type="module">
			...
			const fileSelected = { name: '' };
            var gui;
            let brushOption, folderAnimations, properties;
            var loaderFile,fileDirs;
            ...
            function start (){
                ...
                loaderFile = new THREE.FileLoader();
            ...
            function loadFiles(){
                return $.post(mapDir,{op:'load'},(e,r)=>{
            ...
            function loadControls(){
                const arcballGui = {
                    ...
                    populateGui: function () {
                        ...
                        brushOption.add( cameraControls, 'enabled' ).name( 'Enable controls' );
                        // brushOption.add( cameraControls, 'enableGrid' ).name( 'Enable Grid' );
                        brushOption.add( cameraControls, 'enableRotate' ).name( 'Enable rotate' );
                        brushOption.add( cameraControls, 'enablePan' ).name( 'Enable pan' );
                        brushOption.add( cameraControls, 'enableZoom' ).name( 'Enable zoom' );
                        // brushOption.add( cameraControls, 'cursorZoom' ).name( 'Cursor zoom' );
                        // brushOption.add( cameraControls, 'adjustNearFar' ).name( 'adjust near/far' );
                        // brushOption.add( cameraControls, 'scaleFactor', 1.1, 10, 0.1 ).name( 'Scale factor' );
                        // brushOption.add( cameraControls, 'minDistance', 0, 50, 0.5 ).name( 'Min distance' );
                        // brushOption.add( cameraControls, 'maxDistance', 0, 50, 0.5 ).name( 'Max distance' );
                        // brushOption.add( cameraControls, 'minZoom', 0, 50, 0.5 ).name( 'Min zoom' );
                        // brushOption.add( cameraControls, 'maxZoom', 0, 50, 0.5 ).name( 'Max zoom' );
                        // brushOption.add( arcballGui, 'gizmoVisible' ).name( 'Show gizmos' ).onChange( function () {

                        //     cameraControls.setGizmosVisible( arcballGui.gizmoVisible );

                        // } );
                        // brushOption.add( cameraControls, 'copyState' ).name( 'Copy state(ctrl+c)' );
                        // brushOption.add( cameraControls, 'pasteState' ).name( 'Paste state(ctrl+v)' );
                        // brushOption.add( cameraControls, 'reset' ).name( 'Reset' );
                        ...
                        //brush setting
                        brushOption.add( properties, 'size', .01, 1, 0.05 ).name( 'Size' );
                        brushOption.add( properties, 'density', 0.01, 10, 3 ).name( 'Density' );
                        brushOption.add( properties, 'up' ).name( 'Up/Down' );
                    }
                };

                gui = new GUI();
                fileSelected.name = fileDirs[0];

                gui.add( fileSelected, 'name', fileDirs ).name( 'Choose File' ).onChange( function (e) {
                    loadVMap(fileSelected.name);
                } );
                loadVMap(fileSelected.name);
                brushOption = gui.addFolder( 'Brush' );
                folderAnimations = brushOption.addFolder( 'Animations' );
            ...
            function initEvents() {
                ...
                document.getElementById("load").addEventListener('click',()=>{loadVMap()})
            }

            function loadVMap(map){
                loaderFile.load(mapDir+map,function(json){
                ...
                if( intersects.length > 0 && intersects[0].object.geometry ){                    
                    ...
                    for (let i = 0; i  < geometry.attributes.position.count; i++) {
                        ...
                        if (distance < properties.size) {
                            if(properties.up)
                                geometry.attributes.position.setY(i, geometry.attributes.position.getY(i) + (properties.size - distance) / properties.density)
                            else{                                
                                geometry.attributes.position.setY(i, geometry.attributes.position.getY(i) - (properties.size - distance) / properties.density)
                            }
                        }
            ...
            function saveJson(){
                // var result=terrainMesh.toJSON();
                // var output =JSON.stringify(result);

                var vertices = terrainMesh.geometry.attributes.position.array;
                var expr = [];
                for(var i=1;i<vertices.length;i+=3){
                    expr.push(vertices[i]);
                }

                var epor1 = {data:expr};
                var jsonString = JSON.stringify( epor1);

                saveArrayBuffer(jsonString, 'scene.json', 'application/json');
            }
